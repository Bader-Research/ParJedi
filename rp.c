...
#include "include/rp_int.h"
#include "include/jedi_int.h"
#include "simple.h"

double determine_cost();
 double OldCosts[20];
 double NewCosts[20];
 double change_in_costs[20];
void
combinatorial_optimize (int (*cost_function)(),
...
int problem_size,
int verbose, THREADED)

  {
    double max_gen, cur_gen, temp_points;
    ...
    OldCost = determine_cost(cost_function, TH);
   ...
   on_one_thread {
	...
	(void) fprintf(Fout, "starting temperature is %g\n", start_temp);
	(void) fprintf(Fout, "starting cost is %g\n", start_cost);
	(void) fprintf(Fout, "\n");
	(void) fflush(Fout);
   ...
   while (!stopping_criterion(c1, c2, c3, c4, stop_temp, temp)) {
	...
	max_gen = (double)problem_size * NewStatesPerUnit;
   cur_gen = 0;
	temp_points++;
	...
	while (!inner_loop_criterion(max_gen, cur_gen)) {	
	  ...
	  generate_new_state(RangeSmall,temp_points,generate_function,TH);
     cur_gen ++;
     if (should_accept(temp,cost_function,TH)) {
		accept_new_state(accept_state,TH); 
	    } else {
   ...
   OldCost = determine_cost(cost_function); 
    ...
    (void) fprintf(Fout, "New temperature is %.4g; ", temp);
    (void) fprintf(Fout, "New cost is %g; ", OldCost);
    (void) fprintf(Fout, "Changed after %g states\n", cur_gen);
    (void) fflush(Fout);
   } }
     node_Barrier();
   ...
   on_one_thread {
	...
	(void) fprintf(Fout, "Starting cost was %g\n", start_cost);
	(void) fprintf(Fout, "Final condition:\n");
	(void) fprintf(Fout, "Final temperature was %.4g; ", temp);
	(void) fprintf(Fout, "Final cost was %g\n", OldCost);
   (void) fprintf(Fout, "\n");
	(void) fprintf(Fout, "Stopped after %g temperature points\n",temp_points);
	(void) fprintf(Fout, "\n");
   (void) fflush(Fout);
   }
   node_Barrier();
...
should_accept(double temp, int (*cost_function)(),THREADED)

{
    double change_in_cost, R, Ra, Y, random_generator();
    ...
    if (change_in_cost < 0) {
     return (TRUE);
        ...
        Y = exp(-change_in_cost/temp);
       	R = (double) int_random_generator(0, 100000);
        R = R/100000;
...
int update_temp(temp, start_temp)
double temp;
double start_temp;
{
...
accept_new_state(int (*accept_state)(),THREADED)
...
reject_new_state(int (*reject_state)())
...
double num_states_per_stage, num_states_generated;
{
...
double determine_cost(int (*cost_function)())
...
generate_new_state(int range, double points, int (*generate_function)(),THREADED)

{
    int ret_val;
  
   
    ret_val = (*generate_function)(range,points,TH);
