...
{ 
	    ...
	    (void) combinatorial_optimize(cost_function, generate_function,
	      ...
	      maximumTemperature, enumtypes[0].ns-1,  verboseFlag,TH);
   
  return 0;
} /* end of opt_embedding */


cost_function(double *ret_cost)

{   int i, j, k=0, m;
    int cost;
    int inc_cost,total_cost;
    int dist;
    int we;
    int ns;
...
generate_function(int range,double points,THREADED)
